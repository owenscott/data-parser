...
for (i in data ) {
	...
	for (j in desiredFields) {
		...
		if (d.merge[field].match) {
			tempD[field] = d.merge[field].value;
		}
		else if (d.a[field]) {
			tempD[field] = d.a[field];
		}
		else if (d.b[field]) {
			tempD[field] = d.b[field];
		}
		else {
			tempD[field] = '';
		}
	...
	for (j in d.arrays.locations.merge) {
		locationObj = {};
		location = d.arrays.locations.merge[j].value.split('|');
	...
	var otherLocations = d.arrays.locations.a.concat(d.arrays.locations.b);
